...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        wget -N --no-check-certificate -O x-ui-${release}-${arch}.tar.gz https://github.com/Gafakon/x-ui/releases/latest/download/x-ui-${release}-${arch}.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/alireza0/x-ui/releases/latest/download/x-ui-${release}-${arch}.tar.gz"
